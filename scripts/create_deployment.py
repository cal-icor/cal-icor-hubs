...
def create_pr(github_user, hub_name, root_path, branch_name):
    """
...
def create_deployment(config, github_user, root_path, manual_config=False):
    ...
    create_label(config["hub_name"], root_path)

    # Create a pull request for the new hub
    create_pr(github_user, config["hub_name"], root_path, branch_name)
